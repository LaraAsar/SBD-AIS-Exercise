...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("Unable to download receipt from S3", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to download receipt")
			return
		}
		defer obj.Close()
		...
		_, err = io.Copy(w, obj)
		if err != nil {
			slog.Error("Unable to send receipt file", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to send receipt")
			return
		}
	}
}
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = s3.PutObject(
			r.Context(),
			storage.OrdersBucket, // bucket name
			dbOrder.Filename(),   // file name
			reader,               // file contents
			int64(len(content)),  // size in bytes
		...
		if err != nil {
			...
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to upload receipt")
			return
		}
		render.Status(r, http.StatusOK)
		render.JSON(w, r, "ok")
